...
def index(request):  
    ...
    dev = deviceForm()
    al1=ServicesForm()
    al2=RequirementsForm()
    al3=documentationForm()
    al4=UtilizedForm()
    al5=TrainingForm()
    al6=EquipmentForm()
    if request.method == 'POST':
        ...
        est= estabForm(request.POST,request.FILES)
        ...
        al1=ServicesForm(request.POST)
        al2=RequirementsForm(request.POST)
        al3=documentationForm(request.POST)
        al4=UtilizedForm(request.POST)
        al5=TrainingForm(request.POST)
        al6=EquipmentForm(request.POST)
        if fm.is_valid() and cm.is_valid():
            ...
            al5=TrainingForm()
            al6=EquipmentForm()
    return render(request,"index.html",{'form':fm,'corm':cm,'forn':fn,'worm':wo,'rorm':re,'qorm':eq,'est':est,'rec':rec,'emp':emp,'ser':ser,'dell':dell,'dev':dev,'al1':al1,'al2':al2,'al3':al3,'al4':al4,'al5':al5,'al6':al6})  
